namespace ThanhDV.SaveKeeper.Common
{
    public static class Constant
    {
        public const string DEFAULT_FILE_SAVE_EXTENSION = ".sav";
        public const string DEFAULT_FILE_META_EXTENSION = ".meta";
        public const string DEFAULT_FILE_NAME = "Default";
        public const string DEFAULT_PROFILE_ID = "PROFILE A";

        public const string FILE_TEMP_EXTENTION = ".tmp";
        public const string FILE_BACKUP_EXTENTION = ".bak";

        public const string ADDRESSABLE_GROUP = "SaveKeeper";

        public const string DEFAULT_SAVE_SETTINGS_SO_FOLDER = "Assets/Plugins/SaveKeeper/SO";
        public const string SAVE_SETTINGS_SO_NAME = "SaveSettings";
        public const string SAVE_SETTINGS_SO_EXTENSION = ".asset";

        public const string SESSION_KEY_CHECKED = "ThanhDV.AudioManager.FMOD.CheckedInSession";
    }
}
