...
SaveKeeper saver         = new SaveKeeper(registry, store, ser, enc, settings);
...
// Fires on the SynchronizationContext where SaveKeeper was constructed (typically Unity main thread).
```

### Async handles

All async APIs return `SaveKeeperOperationHandle` (or `SaveKeeperOperationHandle<T>`):
...
Construct `SaveKeeper` (and therefore `JsonSerializer`) at a splash / bootstrap / loading screen, not during gameplay. Subsequent saves and loads do not re-scan.
...
| Type | Purpose |
|---|---|
| `SaveKeeper` | Main facade. Save/Load/Delete + SimpleData. |
...
| `SaveKeeperOperationHandle` | Awaitable handle returned by async APIs. |
