...
{
    /// <summary>
    /// Static PlayerLoop-driven dispatcher that ticks every active GameSaver instance each Update frame.
    ...
    internal static class AutoSaveTicker
    {
        ...
        private static readonly List<GameSaver> _subscribers = new();
        private static readonly object _lock = new();
        private static bool _installed;

        /// <summary>
        /// Registers a GameSaver for per-frame ticks. Installs the PlayerLoop hook on first subscriber.
        /// </summary>
        public static void Subscribe(GameSaver gameSaver)
        {
            lock (_lock)
            {
                if (_subscribers.Contains(gameSaver)) return;
                _subscribers.Add(gameSaver);
        ...
        /// <summary>
        /// Unregisters a GameSaver. Removes the PlayerLoop hook when the last subscriber leaves.
        /// </summary>
        public static void Unsubscribe(GameSaver gameSaver)
        {
            lock (_lock)
            {
                _subscribers.Remove(gameSaver);
        ...
        private static void OnTick()
        {
            ...
            foreach (GameSaver saver in snapshot)
            {
